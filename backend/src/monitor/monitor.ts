 ...
 */
const ALERT_THRESHOLD = parseInt(process.env.TREASURY_ALERT_THRESHOLD || '5000000', 10);

/**
 * How often the monitor cycle runs (milliseconds).
 * Defaults to 60 000 ms (1 minute).
 */
const POLL_INTERVAL_MS = parseInt(process.env.MONITOR_INTERVAL_MS || '60000', 10);
 ...
 */
const BURN_WINDOW_DAYS = parseInt(process.env.MONITOR_BURN_WINDOW_DAYS || '30', 10);

export const calculateRunwayDays = (balance: number, liabilities: number): number | null => {
 ...
 */
export const computeTreasuryStatus = async (): Promise<EmployerTreasuryStatus[]> => {
    ...
    const liabilityMap = new Map<string, number>(
        liabilities.map((l: TreasuryLiability) => [l.employer, parseFloat(l.liabilities)])
    );

    // Union of all known employers
    const employers = new Set<string>([...balanceMap.keys(), ...liabilityMap.keys()]);
    ...
    return Array.from(employers).map(employer => {
        ...
        return { employer, balance, liabilities: liab, runway_days, alert_sent: false };
    });
};
    ...
    for (const status of statuses) {
        ...
        if (alertNeeded) {
            ...
            try {
                ...
                const msg = err instanceof Error ? err.message : String(err);
                console.error(`[Monitor] Alert delivery failed for ${status.employer}: ${msg}`);
            }
        }
        ...
        try {
            ...
            const msg = err instanceof Error ? err.message : String(err);
            console.error(`[Monitor] Failed to log event for ${status.employer}: ${msg}`);
        }
    }

    console.log(`[Monitor] ✅ Cycle complete — checked ${statuses.length} employer(s)`);
    return statuses;
};
...
export const startMonitor = async (): Promise<void> => {
    if (!getPool()) {
        console.warn('[Monitor] ⚠️  Database not configured — treasury monitor disabled.');
        return;
    }
