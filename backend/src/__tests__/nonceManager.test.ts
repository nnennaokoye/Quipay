...
describe('NonceManager', () => {
    ...
    const testAccountId = 'GAXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX';
    ...
    test('should handle concurrent getNonce requests perfectly sequentially', async () => {
        ...
        const promises = Array.from({ length: numRequests }).map(() => nonceManager.getNonce());
        const nonces = await Promise.all(promises);

        // Verify we got 50 distinct sequential numbers starting from 101 to 150
        const expectedNonces = Array.from({ length: numRequests }).map((_, i) => (101 + i).toString());
